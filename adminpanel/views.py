...
class buy(APIView):
    ...
    def post(self , request, format=None):
        if request.data['act'] == 'reject':
            ...
            return Response(status=status.HTTP_201_CREATED)
        req = buyrequest.objects.get(id = request.data['id'])
        profit = ProfitList(user = req.user , amount = int(req.ramount) - int(req.rramount), currency = 'ریال' , operation = f'{req.currency}خرید')
        profit.save()
...
class buyout(APIView):
    ...
    def post(self , request, format=None):
        if request.data['act'] == 'reject':
            ...
            return Response(status=status.HTTP_201_CREATED)
        req = buyoutrequest.objects.get(id = request.data['id'])
        profit = ProfitList(user = req.user , amount = int(req.ramount) - int(req.rramount) , currency = 'ریال' , operation = f'{req.currency} خرید خارجی')
        profit.save()
        note = Notification(user=req.user, title = 'خرید موفق' , text = ' درخواست خرید شما با موفقیت انجام شد . ')
        note.save()
        req.act = 2
...
class sellout(APIView):
    ...
    def post(self , request, format=None):
        if request.data['act'] == 'reject':
            ...
            return Response(status=status.HTTP_201_CREATED)
        req = selloutrequest.objects.get(id = request.data['id'])
        profit = ProfitList(user = req.user , amount = int(req.rramount) - int(req.ramount), currency = 'ریال' , operation = f'{req.currency} فروش خارجی')
        profit.save()
        note = Notification(user=req.user, title = 'خرید موفق' , text = ' درخواست خرید شما با موفقیت انجام شد . ')
        note.save()
        req.act = 2
