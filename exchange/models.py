...
class UserInfo(models.Model):
    ...
    last_visit = models.DateTimeField(default=timezone.now(tz=utc))
...
class Review(models.Model):
    date = models.DateTimeField(default=timezone.now(tz=utc))
...
class buyrequest(models.Model):
    ...
    def get_age(self):
        ...
        hours=0
        minutes=0
        dif = (timezone.now(tz=utc)- self.date).total_seconds()
        while (dif > 86400):
            dif = dif - 86400
            days = days + 1
        while (dif > 3600):
...
class Transactions(models.Model):
    date = models.DateField(default=timezone.now(tz=utc)) 
    amount = models.FloatField()
