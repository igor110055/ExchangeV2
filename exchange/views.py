...
class buy(APIView):
    ...
    def post(self , request, format=None):
        ...
        if serializer.is_valid():
            ...
            wallet.save()
            serializer.save()
            sms(user = User.objects.get(id = 1) ,text= ' درخواست خرید مقدار' + request.data['camount'] + 'از ارز' + request.data['currency'] + 'برای ' + request.user.username, pattern= 'tfpvvl8beg')
            return Response(serializer.data , status=status.HTTP_201_CREATED)
        print(serializer.errors)
        return Response(serializer.errors , status=status.HTTP_400_BAD_REQUEST)
...
class buyout(APIView):
    ...
    def post(self , request, format=None):
        ...
        if serializer.is_valid():
            ...
            wallet.save()
            serializer.save()
            sms(user = User.objects.get(id = 1)  ,text= ' درخواست خرید ولت خارجی به مقدار' + request.data['camount'] + 'از ارز' + request.data['currency'] + 'برای ' + request.user.username, pattern= 'tfpvvl8beg')
            return Response(serializer.data , status=status.HTTP_201_CREATED)
        print(serializer.errors)
        return Response(serializer.errors , status=status.HTTP_400_BAD_REQUEST)
...
class sellout(APIView):
    ...
    def post(self , request, format=None):
        ...
        if serializer.is_valid():
            serializer.save()
            sms(user = User.objects.get(id = 1) ,text= ' درخواست فروش ولت خارجی به مقدار' + request.data['camount'] + 'از ارز' + request.data['currency'] + 'برای ' + request.user.username, pattern= 'r4hxan3byx')
            return Response(serializer.data , status=status.HTTP_201_CREATED)
        print(serializer.errors)
        return Response(serializer.errors , status=status.HTTP_400_BAD_REQUEST)
