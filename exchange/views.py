...
class cp_transfer(APIView):
    ...
    def post(self , request):
        coinex = CoinEx(Perpetual.objects.get(user=request.user).apikey, Perpetual.objects.get(user=request.user).secretkey )
        return Response(coinex.margin_transfer(from_account=request.data['fa'], to_account=request.data['ta'], coin_type=request.data['coin'] , amount=str(request.data['amount'])))
